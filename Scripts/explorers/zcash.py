...
def scrapePageOfBlocks(p, headers):
    url = "https://zcash.tokenview.com/api/blocks/zcash/%d/100" % p
    ...
    return list(map(lambda b: {
        ...
        "Sent value": None if b["sentValue"] == "" else float(b["sentValue"]),
        "Miner address": b["miner"] if "miner" in b else None,
        "Fee": None if b["fee"] == "" else float(b["fee"]),
        "Reward": None if b["reward"] == "" else float(b["reward"]),
...
def scrapeBlocks():
    ...
    while True:
        ...
        if p % 10 == 0:
            with open(baseDir + "/Data/OtherChains/zcash/historical_blocks.csv", "a") as dest:
            ...
            with open(baseDir + "/Logs/zcash/blocks.txt", "a") as logfile:
